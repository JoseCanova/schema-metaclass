...
public class ThirdNormalFormClassificationTask implements TableClassificationTask<ClassificationData , ThirdNormalFormClassificationResult>{
	...
	private Optional<ThirdNormalFormClassificationResult> evaluateTableIndexResult(Optional<Table> oTable, TableIndexResult theResult) {
		if (theResult.columnsIndexResult().size() == oTable.get().getColumns().size() && columnPresentSingleIndex(oTable))
			return Optional.of(new ThirdNormalFormClassificationResult(oTable.get().getName(),  theResult));
		else 
	...
	private boolean columnPresentSingleIndex(Optional<Table> oTable) {
		return	oTable
				.get()
				.getIndexes()
				.stream()
				.filter(i ->i.isUnique())
				.filter(i -> i.getColumns().size() == 1)
				.count() == oTable.get().getColumns().size();
	}
